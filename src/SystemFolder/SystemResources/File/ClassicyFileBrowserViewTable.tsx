...
export const ClassicyFileBrowserViewTable: React.FC<ClassicyFileBrowserViewTableProps> = ({
    ...
    const fileList = useMemo<ClassicyFileSystemEntryMetadata[]>(() => {
        const a: [string, ClassicyFileSystemEntryMetadata][] = Object.entries(fs.filterByType(path, ['file', 'directory']))
        ...
        return Object.entries(directoryListing).map((entry, index) => {
            ...
            const filteredKeyArray: [string, string][] = Object.entries(entry[1]).filter((key) => key[0].startsWith('_'))
