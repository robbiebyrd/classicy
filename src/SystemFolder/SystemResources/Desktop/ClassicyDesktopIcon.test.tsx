...
vi.mock("@/SystemFolder/ControlPanels/AppManager/ClassicyAppManagerUtils", () => ({
  useAppManager: (selector: Function) => {
    const mockState = {
      System: {
        Manager: {
          Desktop: {
...
vi.mock("@/SystemFolder/SystemResources/Desktop/ClassicyDesktopIcon.scss", () => ({}));
...
describe("ClassicyDesktopIcon", () => {
  ...
  it("calls dispatch with ClassicyDesktopIconFocus on single click", async () => {
    const { container } = render(<ClassicyDesktopIcon {...defaultProps} />);
    const iconDiv = container.querySelector("#TestApp\\.shortcut") as HTMLElement;
