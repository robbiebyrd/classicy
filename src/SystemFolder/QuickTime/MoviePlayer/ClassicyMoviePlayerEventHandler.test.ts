...
function makeStoreWithMoviePlayer(): ClassicyStore {
  const ds = makeStore();
  ds.System.Manager.App.apps["MoviePlayer.app"] = {
    id: "MoviePlayer.app",
    name: "Movie Player",
    icon: "movie-icon.png",
    open: false,
...
describe("classicyQuickTimeMoviePlayerEventHandler — ClassicyAppMoviePlayerOpenDocument", () => {
  it("adds a document to openFiles", () => {
    ...
    expect(result.System.Manager.App.apps["MoviePlayer.app"].data["openFiles"]).toHaveLength(1);
    expect(result.System.Manager.App.apps["MoviePlayer.app"].data["openFiles"][0].url).toBe(doc.url);
  });
  ...
  it("is a no-op when document is undefined", () => {
    ...
    expect(result.System.Manager.App.apps["MoviePlayer.app"].data["openFiles"]).toHaveLength(0);
  });
...
describe("classicyQuickTimeMoviePlayerEventHandler — ClassicyAppMoviePlayerOpenDocuments", () => {
  it("bulk adds multiple documents", () => {
    ...
    expect(result.System.Manager.App.apps["MoviePlayer.app"].data["openFiles"]).toHaveLength(2);
  });

  it("filters out duplicates when bulk adding", () => {
    const ds = makeStoreWithMoviePlayer();
    ds.System.Manager.App.apps["MoviePlayer.app"].data["openFiles"] = [
      { url: "http://example.com/a.mp4", name: "A" },
    ];
    ...
    expect(result.System.Manager.App.apps["MoviePlayer.app"].data["openFiles"]).toHaveLength(2);
    const urls = result.System.Manager.App.apps["MoviePlayer.app"].data["openFiles"].map(
      (f: { url: string }) => f.url,
    );
...
describe("classicyQuickTimeMoviePlayerEventHandler — ClassicyAppMoviePlayerCloseDocument", () => {
  it("removes a document by URL", () => {
    ...
    expect(result.System.Manager.App.apps["MoviePlayer.app"].data["openFiles"]).toHaveLength(1);
    expect(result.System.Manager.App.apps["MoviePlayer.app"].data["openFiles"][0].url).toBe(
