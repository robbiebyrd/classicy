...
export const getTheme = (theme: string, overrides?: {}) => {
    const namedThemeData = themesData.find((t) => t.id === theme) || themesData[0]
    const updatedTheme = overrides ? mergeDeep(namedThemeData, overrides) : namedThemeData
    return updatedTheme as ClassicyTheme
}

export const mergeDeep = (target: Record<string, any>, ...sources: any[]): Object => {
    if (!sources.length) {
        return target
    }
    ...
    const isObject = (item: Object | Array<any>) => {
