...
function makeValidStoredState(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    System: {
      Manager: {
        App: { apps: {} },
        Desktop: { selectedIcons: [], contextMenu: [], showContextMenu: false, icons: [], systemMenu: [], appMenu: [], selectBox: { size: [0, 0], start: [0, 0], active: false } },
        Appearance: { activeTheme: { id: "default" }, availableThemes: [] },
        Sound: { volume: 100, labels: {}, disabled: [] },
        DateAndTime: { show: true, dateTime: new Date().toISOString(), timeZoneOffset: "0", militaryTime: false, displaySeconds: true, displayPeriod: true, displayDay: true, displayLongDay: false, flashSeparators: false },
      },
    },
    ...overrides,
  };
...
describe("dispatch", () => {
  ...
  beforeEach(async () => {
    vi.resetModules();
    localStorage.clear();
    const mod = await import("@/SystemFolder/ControlPanels/AppManager/ClassicyAppManagerUtils");
  ...
  it("ClassicyAppOpen adds the app to state with open=true", () => {
    dispatch({ type: "ClassicyAppOpen", app: { id: "Calculator.app", name: "Calculator", icon: "" } });
  ...
  it("ClassicyAppClose marks the app as closed", () => {
    dispatch({ type: "ClassicyAppOpen", app: { id: "Notes.app", name: "Notes", icon: "" } });
  ...
  it("multiple dispatches accumulate correctly", () => {
    dispatch({ type: "ClassicyAppOpen", app: { id: "App1.app", name: "App One", icon: "" } });
    dispatch({ type: "ClassicyAppOpen", app: { id: "App2.app", name: "App Two", icon: "" } });
...
describe("persistence lifecycle", () => {
  ...
  it("writes state to localStorage after 500ms debounce", () => {
    dispatch({ type: "ClassicyAppOpen", app: { id: "Test.app", name: "Test", icon: "" } });
  ...
  it("stopAppManagerPersistence prevents further writes", () => {
    ...
    dispatch({ type: "ClassicyAppOpen", app: { id: "Another.app", name: "Another", icon: "" } });
    vi.advanceTimersByTime(500);
...
describe("getInitialState — localStorage behaviour (via module re-import)", () => {
  ...
  it("restores valid persisted state from localStorage", async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- manipulating raw JSON structure for localStorage test
    (stored.System as any).Manager.App.apps["Persisted.app"] = {
      id: "Persisted.app", name: "Persisted", icon: "", windows: [], open: true,
    };
  ...
  it("falls back to DefaultAppManagerState and calls console.warn when System.Manager.App is missing", async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- intentionally corrupting state for schema validation test
    delete (badState.System as any).Manager.App;
  ...
  it("falls back to DefaultAppManagerState and calls console.warn when Appearance.activeTheme is missing", async () => {
    const badState = makeValidStoredState();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- intentionally corrupting state for schema validation test
    delete (badState.System as any).Manager.Appearance.activeTheme;
    localStorage.setItem("classicyDesktopState", JSON.stringify(badState));
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
  ...
  it("falls back when System key is absent entirely", async () => {
    localStorage.setItem("classicyDesktopState", JSON.stringify({ NotSystem: {} }));
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
