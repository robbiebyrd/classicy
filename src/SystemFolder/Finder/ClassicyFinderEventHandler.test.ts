...
describe("classicyFinderEventHandler — ClassicyAppFinderOpenFolder", () => {
  ...
  it("adds path to existing openPaths", () => {
    const ds = makeStore();
    ds.System.Manager.App.apps["Finder.app"].data = { openPaths: ["/Users/existing"] };
  ...
  it("deduplicates when the same path is added twice", () => {
    const ds = makeStore();
    ds.System.Manager.App.apps["Finder.app"].data = { openPaths: ["/Users/test"] };
...
describe("classicyFinderEventHandler — ClassicyAppFinderOpenFolders", () => {
  it("merges multiple paths into openPaths", () => {
    ...
    expect(openPaths(ds)).toEqual(expect.arrayContaining(["/Users/a", "/Users/b"]));
    expect(openPaths(ds)).toHaveLength(2);
  });
  ...
  it("handles empty appData gracefully", () => {
    const ds = makeStore();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- intentionally passing null to test guard path
    ds.System.Manager.App.apps["Finder.app"].data = null as any;
...
describe("classicyFinderEventHandler — no-op actions", () => {
  ...
  it("ClassicyAppFinderOpenFile does not throw and returns ds", () => {
    ...
    expect(() =>
      classicyFinderEventHandler(ds, { type: "ClassicyAppFinderOpenFile", path: "/Users/file.txt" })
    ).not.toThrow();
  });
});
